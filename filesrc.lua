...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>đăng khoa hubchào ae<Color=/>"):Display()
    wait(.5)
...
local Window = OrionLib:MakeWindow({Name = "đăng khoa hub", HidePremium = false, IntroText = "đăng khoa hub⚜️", SaveConfig = true, ConfigFolder = "NightHub"})
...
W:AddLabel("đăng khoa hubSCRIPT")
W:AddLabel("Discordđăng khoa hub")
local Section = W:AddSection({
    Name = "Status"
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "đăng khoa hub(Noitification)",
